...
int __iox_path_resolve(const char *path, char *resolved, size_t resolved_len) {
    ...
    /* Classify the path */
    iox_path_type_t type = __iox_path_classify(path);
    
    /* If it's a real iOS path (own sandbox or external), use directly */
    if (type == IOX_PATH_OWN_SANDBOX || type == IOX_PATH_EXTERNAL) {
    ...
    if (type == IOX_PATH_VIRTUAL_LINUX) {
        /* First normalize the virtual path */
        char normalized[IOX_MAX_PATH];
    ...
    type = __iox_path_classify(resolved);
    if (type == IOX_PATH_VIRTUAL_LINUX) {
        /* It's a virtual path, translate it */
        char translated[IOX_MAX_PATH];
        if (iox_vfs_translate(resolved, translated, sizeof(translated)) == 0) {
    ...
    if (is_absolute) {
        *dst++ = '/';
        ...
        if (src[0] == '.' && (src[1] == '/' || src[1] == '\0')) {
            src++;  /* Skip the dot */
            while (*src == '/') src++;  /* Skip following slashes */
        }
    }
...
static void __iox_path_init_regex(void) {
    if (regex_initialized) return;
    
    /* Device sandbox: /var/mobile/Containers/Data/Application/<UUID>/ */
    regcomp(&ios_sandbox_regex, 
            "^/var/mobile/Containers/Data/Application/[A-Fa-f0-9-]+/",
            REG_EXTENDED | REG_NOSUB);
    
    /* Simulator sandbox: ~/Library/Developer/CoreSimulator/Devices/<SIM-UUID>/data/Containers/Data/Application/<APP-UUID>/ */
    regcomp(&ios_simulator_regex,
            "/Library/Developer/CoreSimulator/Devices/[A-Fa-f0-9-]+/data/Containers/Data/Application/[A-Fa-f0-9-]+/",
            REG_EXTENDED | REG_NOSUB);
    
    /* External paths: file-provider, iCloud, shared containers */
    regcomp(&ios_external_regex,
            "^(/private/var/mobile/Library/Mobile Documents/|/private/var/mobile/Containers/Shared/AppGroup/|file-provider://)",
            REG_EXTENDED | REG_NOSUB);
...
bool __iox_path_is_virtual_linux(const char *path) {
    ...
    if (strcmp(path, "/home") == 0) return true;
    if (strncmp(path, "/tmp/", 5) == 0) return true;
    if (strcmp(path, "/tmp") == 0) return true;
    if (strncmp(path, "/etc/", 5) == 0) return true;
    if (strcmp(path, "/etc") == 0) return true;
    if (strncmp(path, "/var/", 5) == 0) return true;
    if (strcmp(path, "/var") == 0) return true;
    if (strncmp(path, "/usr/", 5) == 0) return true;
    if (strcmp(path, "/usr") == 0) return true;
    if (strncmp(path, "/bin/", 5) == 0) return true;
    if (strcmp(path, "/bin") == 0) return true;
    if (strncmp(path, "/sbin/", 6) == 0) return true;
    if (strcmp(path, "/sbin") == 0) return true;
    if (strncmp(path, "/lib/", 5) == 0) return true;
    if (strcmp(path, "/lib") == 0) return true;
    if (strncmp(path, "/root/", 6) == 0) return true;
...
bool __iox_path_is_external(const char *path) {
    ...
    if (!__iox_path_is_virtual_linux(path) && !__iox_path_is_own_sandbox(path)) {
...
iox_path_type_t __iox_path_classify(const char *path) {
    if (!path || !*path) {
        return IOX_PATH_INVALID;
    }
    
    if (__iox_path_is_virtual_linux(path)) {
        return IOX_PATH_VIRTUAL_LINUX;
    }
    
    if (__iox_path_is_own_sandbox(path)) {
        return IOX_PATH_OWN_SANDBOX;
    }
    
    if (__iox_path_is_external(path)) {
        return IOX_PATH_EXTERNAL;
    }
...
int __iox_path_virtual_to_ios(const char *vpath, char *ios_path, size_t ios_path_len) {
    ...
    if (!__iox_path_is_virtual_linux(vpath)) {
...
bool __iox_path_is_direct(const char *path) {
    ...
    iox_path_type_t type = __iox_path_classify(path);
    return (type == IOX_PATH_OWN_SANDBOX || type == IOX_PATH_EXTERNAL);
}
