...
# Automated Xcode project setup for libiox testing
#

set -e

PROJECT_NAME="IOSTestApp"
BUNDLE_ID="com.libiox.test"
IOS_VERSION="16.0"

echo "Setting up Xcode project for libiox testing..."
...
extern int iox_open(const char *pathname, int flags, mode_t mode);
extern ssize_t iox_read(int fd, void *buf, size_t count);
extern ssize_t iox_write(int fd, const void *buf, size_t count);
extern int iox_close(int fd);
...
TEST(init) {
    ASSERT_TRUE(iox_is_initialized());
    const char *ver = iox_version();
    ASSERT_NE(ver, NULL);
    ASSERT_GT(strlen(ver), 0);
    NSLog(@"libiox version: %s", ver);
}
...
TEST(file_io) {
    // Create a test file
    const char *test_path = "/tmp/iox_test_file.txt";
    const char *test_data = "Hello from libiox!";
    
    // Write
    int fd = iox_open(test_path, O_CREAT | O_WRONLY | O_TRUNC, 0644);
    ASSERT_GT(fd, 0);
    
    ssize_t written = iox_write(fd, test_data, strlen(test_data));
    ASSERT_EQ(written, strlen(test_data));
    
    iox_close(fd);
    
    // Read back
    fd = iox_open(test_path, O_RDONLY, 0);
    ASSERT_GT(fd, 0);
    
    char buf[256];
    ssize_t read_bytes = iox_read(fd, buf, sizeof(buf));
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (tests_failed == 0) {
        label.text = [NSString stringWithFormat:@"libiox Tests\n\n✓ All %d tests passed!", tests_passed];
            ...
            <objects>
                <viewController id="01J-lp-oVM" sceneMemberID="viewController">
                    <view key="view" contentMode="scaleToFill" id="Ze5-6b-2t3">
                        ...
                        <subviews>
                            <label opaque="NO" clipsSubviews="YES" userInteractionEnabled="NO" contentMode="left" horizontalHuggingPriority="251" verticalHuggingPriority="251" text="libiox Test" textAlignment="center" lineBreakMode="middleTruncation" baselineAdjustment="alignBaselines" minimumFontSize="18" translatesAutoresizingMaskIntoConstraints="NO" id="GJd-Yh-RWb">
...
echo "4. Set bundle ID: com.libiox.test"
echo "5. Choose location: $(pwd)/$PROJECT_NAME"
echo "6. Replace main.m with the generated main.m"
echo "7. Add libiox-sim.a to project (drag from parent directory)"
