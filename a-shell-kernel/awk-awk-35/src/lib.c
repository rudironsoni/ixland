...
__thread bool	innew;		/* true = infile has not been read by readrec */
__thread char	*file	= EMPTY;
...
static __thread size_t	len_inputFS = 0;
static __thread char	*inputFS = NULL; /* FS at time of input, for field splitting */

#define	MAXFLD	2
__thread int	nfields	= MAXFLD;	/* last allocated slot for $i */

__thread bool	donefld;	/* true = implies rec broken into fields */
__thread bool	donerec;	/* true = record is valid (no flds have changed) */
...
// iOS: not static, so it can be initialized in main()
__thread bool firsttime = true;
