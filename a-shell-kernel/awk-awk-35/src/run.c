...
extern	__thread Awkfloat	srand_seed;
...
extern void clearsymboltables(void);
void run(Node *a)	/* execution of parse tree starts here */
{

	stdinit();
	execute(a);
	closeall(); // iOS: also frees frame and files. We can't free tmps.
	// iOS additions: free everything.
    // freeTree(a, 1); // Does not work anymore with "new" awk.
    // Running hundreds of tests, awk accumulates memory loss.
    // running freeTree does not help (memory loss is the same), but causes a crash.
    clearsymboltables(); // does not free much.
...
extern void freetr_iOS(Node *p);

void freeTree(Node *u, int eraseSelf)    /* iOS: scan the entire tree, and frees the allocated memory */
{
    Node *a;
    Node *anext;
...
{
	static __thread const Cell newcopycell = { OCELL, CCOPY, 0, EMPTY, 0.0, NUM|STR|DONTFREE, NULL, NULL };
	int i, ncall, ndef;
...
FILE *openfile(int a, const char *us, bool *pnewflag)
{
	...
	if (fp != NULL) {
		...
		if (pnewflag)
			*pnewflag = true;
		if (fp != thread_stdin && fp != thread_stdout && fp != thread_stderr)
 ...
 Cell *closefile(Node **a, int n)
 {
	...
	stat = true;
 	for (i = 0; i < nfiles; i++) {
		...
		if (files[i].fp == thread_stdin || files[i].fp == thread_stdout ||
		    files[i].fp == thread_stderr)
...
void closeall(void)
{
	...
	for (i = 0; i < nfiles; i++) {
		...
		if (ferror(files[i].fp))
			FATAL( "i/o error occurred on %s", files[i].fname );
		if (files[i].fp == thread_stdin)
			continue;
		if (files[i].mode == '|' || files[i].mode == LE)
			stat = pclose(files[i].fp) == -1;
		else if (files[i].fp == thread_stdout || files[i].fp == thread_stderr)
			stat = fflush(files[i].fp) == EOF;
		else
			stat = fclose(files[i].fp) == EOF;
		if (stat)
			FATAL( "i/o error occurred while closing %s", files[i].fname );
		// iOS: cleanup and reset values:
        if ((files[i].fp != thread_stdout) && (files[i].fp != thread_stdin) && (files[i].fp != thread_stderr)) {
        	files[i].fp = 0;
            free(files[i].fname);
            files[i].fname = NULL;
        }
