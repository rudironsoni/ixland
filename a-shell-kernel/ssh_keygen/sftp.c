...
static __thread FILE* infile;

/* Are we in batchfile mode? */
static __thread int batchmode = 0;
...
__thread int showprogress = 1;

/* When this option is set, we always recursively download/upload directories */
static __thread int global_rflag = 0;

/* When this option is set, we resume download or upload if possible */
static __thread int global_aflag = 0;

/* When this option is set, the file transfers will always preserve times */
static __thread int global_pflag = 0;

/* When this option is set, transfers will have fsync() called on each file */
static __thread int global_fflag = 0;

/* SIGINT received during command processing */ // Must be the one.
...
help(void)
{
	fprintf(thread_stdout, "Available commands:\n"
	    "bye                                Quit sftp\n"
...
do_df(struct sftp_conn *conn, const char *path, int hflag, int iflag)
{
	...
	if (iflag) {
        fprintf(thread_stdout, "     Inodes        Used       Avail      "
		    "(root)    %%Capacity\n");
        fprintf(thread_stdout, "%11llu %11llu %11llu %11llu         %s\n",
        ...
        fprintf(thread_stdout, "    Size     Used    Avail   (root)    %%Capacity\n");
        fprintf(thread_stdout, "%7sB %7sB %7sB %7sB         %s\n",
		    s_total, s_used, s_avail, s_root, s_dcapacity);
	} else {
        fprintf(thread_stdout, "        Size         Used        Avail       "
		    "(root)    %%Capacity\n");
        fprintf(thread_stdout, "%12llu %12llu %12llu %12llu         %s\n",
    ...
    const char *startdir, int err_abort, int echo_command)
{
	...
	case I_LUMASK:
		umask(n_arg);
        fprintf(thread_stdout, "Local umask: %03lo\n", n_arg);
		break;
	case I_CHMOD:
		path1 = make_absolute(path1, *pwd);
		attrib_clear(&a);
	...
	case I_HELP:
		...
		break;
	case I_VERSION:
        fprintf(thread_stdout, "SFTP protocol version %u\n", sftp_proto_version(conn));
		break;
	case I_PROGRESS:
		showprogress = !showprogress;
		if (showprogress)
            fprintf(thread_stdout, "Progress meter enabled\n");
		else
            fprintf(thread_stdout, "Progress meter disabled\n");
		break;
	default:
...
interactive_loop(struct sftp_conn *conn, char *file1, char *file2)
{
	...
	for (;;) {
		...
		if (el == NULL) {
            if (interactive) {
                fprintf(thread_stdout, "sftp> "); fflush(thread_stdout);
            }
