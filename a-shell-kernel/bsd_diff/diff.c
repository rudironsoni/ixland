...
int	 Nflag, Pflag, diff_rflag, Tflag, diff_cflag;
static int     sflag;
int	 diff_format, diff_context, status, ignore_file_case;
int	 tabsize = 8;
char	*start, *ifdefname, *diffargs, *diff_label[2], *ignore_pats;
...
diff_main(int argc, char **argv)
{
    ...
    if (excludes_list) {
        while (excludes_list) {
            struct excludes *nextEl = excludes_list->next;
            free(excludes_list);
            excludes_list = nextEl;
        }
        excludes_list = NULL;
    }
    regfree(&ignore_re);
