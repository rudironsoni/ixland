...
__thread pid_t __ixland_current_pid = 0;
...
pid_t ixland_getpid(void) {
    if (__ixland_current_pid == 0) {
        pthread_mutex_lock(&pid_lock);
        __ixland_current_pid = next_pid++;
        pthread_mutex_unlock(&pid_lock);
    }
    return __ixland_current_pid;
}
...
int open(const char *pathname, int flags, ...) {
    ...
    return ixland_open(pathname, flags, mode);
}

ssize_t read(int fd, void *buf, size_t count) {
    return ixland_read(fd, buf, count);
}

ssize_t write(int fd, const void *buf, size_t count) {
    return ixland_write(fd, buf, count);
}

int close(int fd) {
    return ixland_close(fd);
}
