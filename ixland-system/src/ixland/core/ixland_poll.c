 ...
 */
static int16_t ixland_poll_events_to_kfilter(short events) {
    int16_t filter = 0;

    if (events & (IXLAND_POLLIN | IXLAND_POLLRDNORM)) {
        filter = EVFILT_READ;
    } else if (events & (IXLAND_POLLOUT | IXLAND_POLLWRNORM)) {
        filter = EVFILT_WRITE;
    }
...
static int ixland_epoll_register_instance(ixland_epoll_instance_t *ep) {
    ...
    for (int i = 0; i < IXLAND_MAX_EPOLL_INSTANCES; i++) {
        if (ixland_epoll_table[i] == NULL) {
            ...
            return i + IXLAND_MAX_FD + 100; /* Reserve range for epoll fds */
        }
    }
    pthread_mutex_unlock(&ixland_epoll_table_lock);
    return -1;
...
static void ixland_epoll_unregister_instance(int epfd) {
    int idx = epfd - IXLAND_MAX_FD - 100;
    if (idx >= 0 && idx < IXLAND_MAX_EPOLL_INSTANCES) {
        pthread_mutex_lock(&ixland_epoll_table_lock);
        ixland_epoll_table[idx] = NULL;
        pthread_mutex_unlock(&ixland_epoll_table_lock);
    }
}
