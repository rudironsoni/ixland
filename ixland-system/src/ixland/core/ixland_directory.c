 ...
 * - ixland_chroot() - Change root directory
...
int ixland_chdir(const char *path) {
    ...
    if (proc) {
        ...
        strncpy(proc->cwd, path, IXLAND_MAX_PATH - 1);
        proc->cwd[IXLAND_MAX_PATH - 1] = '\0';
...
char *ixland_getcwd(char *buf, size_t size) {
    ...
    char vpath[IXLAND_MAX_PATH];
    if (ixland_vfs_reverse_translate(ios_cwd, vpath, sizeof(vpath)) == 0) {
    ...
    if (proc) {
        pthread_mutex_lock(&proc->thread_lock);
        strncpy(proc->cwd, buf, IXLAND_MAX_PATH - 1);
        proc->cwd[IXLAND_MAX_PATH - 1] = '\0';
        pthread_mutex_unlock(&proc->thread_lock);
    }
...
int ixland_mkdir(const char *pathname, mode_t mode) {
    ...
    char parent_path[IXLAND_MAX_PATH];
    strncpy(parent_path, translated_path, IXLAND_MAX_PATH - 1);
    parent_path[IXLAND_MAX_PATH - 1] = '\0';
...
int ixland_symlink(const char *target, const char *linkpath) {
    ...
    /* Check write permission on parent */
    char parent_path[IXLAND_MAX_PATH];
    strncpy(parent_path, translated_link, IXLAND_MAX_PATH - 1);
    parent_path[IXLAND_MAX_PATH - 1] = '\0';
