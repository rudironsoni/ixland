 ...
 * ============================================================================ */
IOX_TEST(lifecycle_fork_wait_exit_basic) {
    ...
    IOX_ASSERT(child->pid > 0);
    IOX_ASSERT(child->pid != parent_pid);
    ...
    int status;
    pid_t waited_pid = iox_waitpid(child_pid, &status, 0);

    /* Verify wait collected correct child */
    IOX_ASSERT_EQ(waited_pid, child_pid);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_multiple_children_wait_any) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        IOX_ASSERT_EQ(children[i]->pgid, parent->pgid);
    }
    ...
    IOX_ASSERT(found_idx >= 0);
    IOX_ASSERT_EQ(WEXITSTATUS(status), 100 + found_idx);

    /* Wait for remaining children */
    int remaining = 2;
    while (remaining > 0) {
        result = iox_waitpid(-1, &status, 0);
        IOX_ASSERT(result > 0);
        IOX_ASSERT(WIFEXITED(status));
        remaining--;
    }
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_wait_specific_pid) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    int status;
    pid_t result = iox_waitpid(pid2, &status, 0);
    IOX_ASSERT_EQ(result, pid2);
    IOX_ASSERT(WIFEXITED(status));
    IOX_ASSERT_EQ(WEXITSTATUS(status), 22);
    ...
    /* Now wait for child1 */
    result = iox_waitpid(pid1, &status, 0);
    IOX_ASSERT_EQ(result, pid1);
    IOX_ASSERT(WIFEXITED(status));
    IOX_ASSERT_EQ(WEXITSTATUS(status), 11);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_exit_status_preservation) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    for (int i = 0; i < num_statuses; i++) {
        ...
        IOX_ASSERT(WIFEXITED(status));
        IOX_ASSERT_EQ(WEXITSTATUS(status), test_statuses[i]);
    }
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_signaled_child_status) {
    IOX_ASSERT(iox_task_init() == 0);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_process_group_inheritance) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        IOX_ASSERT_EQ(child->pgid, original_pgid);
    }
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_session_inheritance) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    IOX_ASSERT_EQ(child->sid, original_sid);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_wnohang_with_exited_children) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    IOX_ASSERT_EQ(result, child_pid);
    IOX_ASSERT(WIFEXITED(status));
    IOX_ASSERT_EQ(WEXITSTATUS(status), 77);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_wnohang_live_children_returns_zero) {
    IOX_ASSERT(iox_task_init() == 0);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_zombie_persists_until_wait) {
    IOX_ASSERT(iox_task_init() == 0);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_exec_preserves_pid) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    strncpy(task->exec_image->path, "/bin/test", IOX_MAX_PATH - 1);
    strncpy(task->comm, "test", IOX_MAX_NAME - 1);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_sequential_fork_wait_cycles) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        IOX_ASSERT_EQ(result, child_pid);
        IOX_ASSERT(WIFEXITED(status));
        IOX_ASSERT_EQ(WEXITSTATUS(status), i);
 ...
 * ============================================================================ */
IOX_TEST(lifecycle_comprehensive_exit_codes) {
    IOX_ASSERT(iox_task_init() == 0);
    ...
    for (int i = 0; i < num_codes; i++) {
        ...
        IOX_ASSERT_EQ(result, child_pid);
        IOX_ASSERT(WIFEXITED(status));
        IOX_ASSERT_EQ(WEXITSTATUS(status), exit_codes[i]);
