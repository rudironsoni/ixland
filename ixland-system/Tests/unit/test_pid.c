...
IOX_TEST(pid_alloc_returns_unique_values) {
    ...
    /* All should be valid */
    IOX_ASSERT_GE(p1, IOX_MIN_PID);
    IOX_ASSERT_GE(p2, IOX_MIN_PID);
    IOX_ASSERT_GE(p3, IOX_MIN_PID);
...
IOX_TEST(pid_free_immediate_reuse) {
    pid_t p1 = iox_alloc_pid();
    IOX_ASSERT_GT(p1, 0);
...
IOX_TEST(pid_range_validation) {
    ...
    for (int i = 0; i < 100; i++) {
        pids[i] = iox_alloc_pid();
        IOX_ASSERT_GT(pids[i], 0);
        IOX_ASSERT_GE(pids[i], IOX_MIN_PID);
        IOX_ASSERT_LE(pids[i], IOX_MAX_PID);
    }
...
IOX_TEST(pid_alloc_free_cycles) {
...
IOX_TEST(pid_uniqueness_no_collisions) {
    const int count = 500;
    pid_t pids[500];
...
IOX_TEST(pid_wraparound_simulation) {
...
IOX_TEST(pid_stress_rapid_allocation) {
    const int count = 10000;
    pid_t *pids = malloc(count * sizeof(pid_t));
    IOX_ASSERT_NOT_NULL(pids);
...
IOX_TEST(pid_o1_timing) {
    ...
    IOX_ASSERT_LT(avg, 10000); /* 10 microseconds in nanoseconds */
...
IOX_TEST(pid_thread_safety) {
    ...
    IOX_ASSERT_EQ(atomic_load(&thread_errors), 0);

    return true;
}

/* Test that invalid PIDs are handled gracefully */
IOX_TEST(pid_invalid_handling) {
    /* Free invalid PIDs should not crash */
    iox_free_pid(0);
    iox_free_pid(1);
    iox_free_pid(999);   /* Just below MIN */
    iox_free_pid(65536); /* Just above MAX */
    iox_free_pid(-1);
