...
@interface IOXMemoryTests : XCTestCase
@end

@implementation IOXMemoryTests
...
- (void)testMmap_LargeAllocation {
    ...
    size_t before = [IOXMemoryTracker currentResidentSize];
    ...
    if (addr != MAP_FAILED) {
        ...
        size_t after = [IOXMemoryTracker currentResidentSize];
        NSLog(@"Memory before: %zu MB, after: %zu MB", before / (1024*1024), after / (1024*1024));
        
        munmap(addr, size);
        
        IOXAssertNoMemoryLeak(@"Large mmap allocation should not leak");
    }
}
...
- (void)testMmap_MultipleAllocations {
    ...
    IOXAssertNoMemoryLeak(@"Multiple mmap allocations should not leak");
}
...
- (void)testMemoryStress_MmapMunmap {
    size_t initialMemory = [IOXMemoryTracker currentResidentSize];
    size_t size = 4096;
    ...
    size_t finalMemory = [IOXMemoryTracker currentResidentSize];
...
- (void)testMemoryStress_MprotectChanges {
    ...
    if (addr != MAP_FAILED) {
        ...
        IOXAssertNoMemoryLeak(@"mprotect stress test should not leak");
    }
}
...
- (void)testMemory_MultipleSizes {
    ...
    IOXAssertNoMemoryLeak(@"Multiple size allocations should not leak");
}
