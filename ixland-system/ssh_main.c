...
static int ssh_verify_host(char *addr, LIBSSH2_SESSION *_session, char *hostname, int port) {
    ...
    char path[PATH_MAX];
    if (getenv("SSH_HOME"))
        strcpy(path, getenv("SSH_HOME"));
    else
        sprintf(path, "%s/Documents/", getenv("HOME"));
    ...
    int kh_key_type = (key_type == LIBSSH2_HOSTKEY_TYPE_RSA) ? LIBSSH2_KNOWNHOST_KEY_SSHRSA
    ...
    if (key) {
        ...
        int check = libssh2_knownhost_checkp(
            kh, hostname, port, key, key_len,
            LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW | kh_key_type, &knownHost);
        if (check == LIBSSH2_KNOWNHOST_CHECK_FAILURE) {
            ...
            fprintf(thread_stderr,
                    ...
                    "(%s) has changed.\nThis might be due to someone doing something nasty or just "
    ...
    if (!succ) {
        // [self authorize_new_key:key length:key_len type:kh_key_type knownHosts:kh
        // filePath:khFilePath]; Add key to the server
        int rc =
            libssh2_knownhost_addc(kh, hostname,
                                   NULL,                   // No hashed addr, no salt
                                   key, key_len, NULL, 0,  // No comment
                                   LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW |
                                       LIBSSH2_KNOWNHOST_KEY_SSHRSA,  // kh_key_type,
...
static int ssh_timeout_connect(int _sock, const struct sockaddr *addr, socklen_t len,
                               int *timeoutp) {
    struct timeval tv;
    fd_set fdset;
    int res;
...
int ssh_main(int argc, char **argv) {
    ...
    for (ai = addrs; ai; ai = ai->ai_next) {
        ...
        if (_sock < 0) {
            ...
            if (!ai->ai_next) {
                fprintf(thread_stderr, "ssh: connect to host %s port %s: %s", host, strport,
                        strerror(errno));
                if (commandLine)
                    free(commandLine);
                return 0;
    ...
    if (auth_type & 4) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            while ((dp = readdir(dirp)) != NULL) {
                ...
                if (strncmp(publickeyName + strlen(publickeyName) - suffix_len, suffix,
                            suffix_len) != 0)
                    continue;
                ...
                if (access(privatekeypath, F_OK) == -1)
                    continue;
                while ((rc = libssh2_userauth_publickey_fromfile_ex(
                            _session, user, strlen(user), publickeypath, privatekeypath,
                            passphrase)) == LIBSSH2_ERROR_EAGAIN)
                    ;
                if (rc != 0) {
                    if (verboseFlag)
                        fprintf(thread_stderr, "Authentication failure with passphrase.\n");
                    continue;
                }  // try another key
                // We are connected
                rc = 0;
                ...
                if (commandLine) {
                    // simple version:
                    while ((rc = libssh2_channel_exec(_channel, commandLine)) ==
                           LIBSSH2_ERROR_EAGAIN) {
                        ssh_waitsocket(_sock, _session);
                    }
                    if (rc != 0) {
                    ...
                    while ((rc = libssh2_channel_request_pty(_channel, "xterm-256color")) ==
                           LIBSSH2_ERROR_EAGAIN) {
                        ssh_waitsocket(_sock, _session);
                    }
                    if (rc) {
            ...
            if (rc != -3) {
                if (i == 0)
                    fprintf(
                        thread_stderr,
                        "Authentication with public key failed. Trying with key + passphrase.\n");
                else
                    fprintf(thread_stderr, "Authentication with public key failed. Please enter "
                                           "another passphrase.\n");
                fflush(thread_stderr);
                if (passphrase)
                    free(passphrase);
                passphrase = read_passphrase(
                    "Please enter passphrase for your public key\n (empty to stop)", 1);
    ...
    if (auth_type & 1) {
        ...
        while (passphrase && (strlen(passphrase) > 0)) {
            while ((rc = libssh2_userauth_password(_session, user, passphrase) ==
                         LIBSSH2_ERROR_EAGAIN))
                ;
            if (rc != 0) {
                if (verboseFlag)
                    fprintf(thread_stderr, "Authentication failure with password.\n");
                continue;
            ...
            if (rc != -3) {
                fprintf(
                    thread_stderr,
                    "Authentication with passphrase failed. Please enter another passphrase.\n");
                fflush(thread_stderr);
                if (passphrase)
                    free(passphrase);
                ...
                if (commandLine)
                    free(commandLine);
                fprintf(thread_stderr,
                        "Authentication with passphrase worked, but failed to execute.\n");
                return rc;
            }
        }
    }
