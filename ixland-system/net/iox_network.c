/* iox_network.c - Network syscall implementation for iOS
 *
...
#define IOX_MAX_SOCKETS 256
static iox_socket_entry_t socket_table[IOX_MAX_SOCKETS];
...
static int iox_socket_alloc(void) {
    pthread_mutex_lock(&socket_table_lock);
    for (int i = 0; i < IOX_MAX_SOCKETS; i++) {
...
static void iox_socket_free(int fd) {
    if (fd < 0 || fd >= IOX_MAX_SOCKETS) return;
    ...
    if (socket_table[fd].used) {
        ...
        memset(&socket_table[fd], 0, sizeof(iox_socket_entry_t));
    }
    pthread_mutex_unlock(&socket_table_lock);
}

static iox_socket_entry_t* iox_socket_get(int fd) {
    if (fd < 0 || fd >= IOX_MAX_SOCKETS) {
        errno = EBADF;
        return NULL;
    }
    if (!socket_table[fd].used) {
...
int __iox_network_init_impl(void) {
...
int __iox_network_deinit_impl(void) {
    if (!atomic_load(&network_initialized)) {
        return 0;
    }
...
int __iox_socket_impl(int domain, int type, int protocol) {
    int fd = iox_socket_alloc();
    if (fd < 0) {
        return -1;
    }
    
    iox_socket_entry_t *sock = &socket_table[fd];
    sock->domain = domain;
    sock->type = type;
...
int __iox_socketpair_impl(int domain, int type, int protocol, int sv[2]) {
...
int __iox_connect_impl(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
int __iox_bind_impl(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
int __iox_listen_impl(int sockfd, int backlog) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
int __iox_accept_impl(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
int __iox_accept4_impl(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags) {
    (void)flags;
    return __iox_accept_impl(sockfd, addr, addrlen);
}
...
ssize_t __iox_send_impl(int sockfd, const void *buf, size_t len, int flags) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
ssize_t __iox_recv_impl(int sockfd, void *buf, size_t len, int flags) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
    ...
    nw_connection_receive(sock->connection, 1, len, 
        ^(dispatch_data_t content, nw_content_context_t context, bool is_complete, nw_error_t error) {
            ...
            if (content) {
                ...
                dispatch_data_t mapped = dispatch_data_create_map(content, &data_ptr, &data_len);
...
ssize_t __iox_sendto_impl(int sockfd, const void *buf, size_t len, int flags,
                         const struct sockaddr *dest_addr, socklen_t addrlen) {
    /* For connectionless sockets */
    (void)dest_addr;
    (void)addrlen;
    return __iox_send_impl(sockfd, buf, len, flags);
}

ssize_t __iox_recvfrom_impl(int sockfd, void *buf, size_t len, int flags,
                           struct sockaddr *src_addr, socklen_t *addrlen) {
    /* For connectionless sockets */
    (void)src_addr;
    (void)addrlen;
    return __iox_recv_impl(sockfd, buf, len, flags);
}
...
int __iox_setsockopt_impl(int sockfd, int level, int optname, 
...
int __iox_getsockopt_impl(int sockfd, int level, int optname,
...
int __iox_inet_pton_impl(int af, const char *src, void *dst) {
    if (af == AF_INET) {
...
const char* __iox_inet_ntop_impl(int af, const void *src, char *dst, socklen_t size) {
    if (af == AF_INET) {
...
int __iox_shutdown_impl(int sockfd, int how) {
    iox_socket_entry_t *sock = iox_socket_get(sockfd);
    if (!sock) {
        return -1;
    }
...
int iox_accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags) {
    return __iox_accept4_impl(sockfd, addr, addrlen, flags);
}
...
ssize_t iox_sendto(int sockfd, const void *buf, size_t len, int flags,
                   const struct sockaddr *dest_addr, socklen_t addrlen) {
    return __iox_sendto_impl(sockfd, buf, len, flags, dest_addr, addrlen);
}

ssize_t iox_recvfrom(int sockfd, void *buf, size_t len, int flags,
                     struct sockaddr *src_addr, socklen_t *addrlen) {
    return __iox_recvfrom_impl(sockfd, buf, len, flags, src_addr, addrlen);
}

int iox_setsockopt(int sockfd, int level, int optname, 
                   const void *optval, socklen_t optlen) {
    return __iox_setsockopt_impl(sockfd, level, optname, optval, optlen);
}

int iox_getsockopt(int sockfd, int level, int optname,
                   void *optval, socklen_t *optlen) {
    return __iox_getsockopt_impl(sockfd, level, optname, optval, optlen);
}

int iox_shutdown(int sockfd, int how) {
    return __iox_shutdown_impl(sockfd, how);
}

int iox_inet_pton(int af, const char *src, void *dst) {
    return __iox_inet_pton_impl(af, src, dst);
}

const char* iox_inet_ntop(int af, const void *src, char *dst, socklen_t size) {
    return __iox_inet_ntop_impl(af, src, dst, size);
}

/* Network initialization - called from iox_init */
int iox_network_init(void) {
    return __iox_network_init_impl();
}

int iox_network_deinit(void) {
    return __iox_network_deinit_impl();
}
