 ...
 * Implements WASI syscall bridge to iox
 */
...
static __wasi_errno_t errno_to_wasi(int err) {
    switch (err) {
        ...
        case EIO: return __WASI_EIO;
        case EBADF: return __WASI_EBADF;
        case ENOMEM: return __WASI_ENOMEM;
        ...
        case EMFILE: return __WASI_EMFILE;
        case EFBIG: return __WASI_EFBIG;
        case ENOSPC: return __WASI_ENOSPC;
        case ESPIPE: return __WASI_ESPIPE;
        case EROFS: return __WASI_EROFS;
        case ENAMETOOLONG: return __WASI_ENAMETOOLONG;
...
static int32_t wasi_args_sizes_get(void *exec_env, int32_t argc_offset, int32_t argv_buf_size_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
...
static int32_t wasi_environ_get(void *exec_env, int32_t environ_offset, int32_t environ_buf_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
...
static int32_t wasi_environ_sizes_get(void *exec_env, int32_t environ_count_offset, int32_t environ_buf_size_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
    ...
    uint32_t *environ_buf_size = wasm_runtime_addr_app_to_native(module_inst, environ_buf_size_offset);
...
static int32_t wasi_fd_write(void *exec_env, int32_t fd, int32_t iovs_offset, int32_t iovs_len, int32_t nwritten_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
...
static int32_t wasi_fd_read(void *exec_env, int32_t fd, int32_t iovs_offset, int32_t iovs_len, int32_t nread_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
...
static int32_t wasi_fd_seek(void *exec_env, int32_t fd, int64_t offset, int32_t whence, int32_t newoffset_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
    ...
    int native_whence;
    switch (whence) {
        case __WASI_WHENCE_SET: native_whence = SEEK_SET; break;
        case __WASI_WHENCE_CUR: native_whence = SEEK_CUR; break;
        case __WASI_WHENCE_END: native_whence = SEEK_END; break;
        default: return __WASI_EINVAL;
    }
...
static int32_t wasi_path_open(void *exec_env, int32_t dirfd, int32_t dirflags, 
                               int32_t path_offset, int32_t path_len, int32_t oflags,
                               int64_t fs_rights_base, int64_t fs_rights_inheriting,
                               int32_t fdflags, int32_t fd_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
    ...
    if (oflags & __WASI_O_CREAT) native_flags |= O_CREAT;
    if (oflags & __WASI_O_DIRECTORY) native_flags |= O_DIRECTORY;
    if (oflags & __WASI_O_EXCL) native_flags |= O_EXCL;
    if (oflags & __WASI_O_TRUNC) native_flags |= O_TRUNC;
    
    if (fs_rights_base & __WASI_RIGHT_FD_READ) native_flags |= O_RDONLY;
    ...
    /* Open through VFS */
    int real_fd = iox_vfs_open(path, native_flags, 0644);
    if (real_fd < 0) {
        return errno_to_wasi(errno);
    }
...
static int32_t wasi_clock_time_get(void *exec_env, int32_t clock_id, int64_t precision, int32_t time_offset) {
    wasm_module_inst_t module_inst = wasm_runtime_get_module_inst(exec_env);
    if (!module_inst) {
        return __WASI_EINVAL;
    }
    ...
    switch (clock_id) {
        case __WASI_CLOCK_REALTIME: native_clock = CLOCK_REALTIME; break;
        case __WASI_CLOCK_MONOTONIC: native_clock = CLOCK_MONOTONIC; break;
        case __WASI_CLOCK_PROCESS_CPUTIME_ID: native_clock = CLOCK_PROCESS_CPUTIME_ID; break;
        case __WASI_CLOCK_THREAD_CPUTIME_ID: native_clock = CLOCK_THREAD_CPUTIME_ID; break;
        default: return __WASI_EINVAL;
    }
...
int iox_wamr_load_wasm(const char *wasm_file, int argc, char **argv) {
    /* Read WASM file */
    ...
    /* Set up WASI args */
    wasm_runtime_set_wasi_args(module_inst, NULL, 0, NULL, 0, 
                               (const char **)argv, argc, NULL, 0);
...
int main(int argc, char **argv) {
    ...
    /* Run WASM */
    int ret = iox_wamr_load_wasm(wasm_file, argc - 1, argv + 1);
