...
@interface IOXTtyTests : XCTestCase
@end

@implementation IOXTtyTests
...
- (void)testIsatty_RegularFile {
    NSString *testFile = [IOXTestHelpers tempFilePathWithPrefix:@"tty_test"];
...
- (void)testTcgetattr_RegularFile {
    NSString *testFile = [IOXTestHelpers tempFilePathWithPrefix:@"tcgetattr_test"];
    const char *path = [testFile UTF8String];
...
- (void)testTcsetattr_RegularFile {
    NSString *testFile = [IOXTestHelpers tempFilePathWithPrefix:@"tcsetattr_test"];
    const char *path = [testFile UTF8String];
...
- (void)testTtyStress_MultipleGetSetCycles {
    ...
    if (getResult == 0) {
        ...
        IOXAssertNoMemoryLeak(@"TTY stress test should not leak");
    }
}
...
- (void)testIsatty_ClosedFd {
    NSString *testFile = [IOXTestHelpers tempFilePathWithPrefix:@"closed_fd_test"];
    const char *path = [testFile UTF8String];
