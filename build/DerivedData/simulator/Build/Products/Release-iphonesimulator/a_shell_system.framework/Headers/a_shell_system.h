...
//! Project version number for a_shell_system.
FOUNDATION_EXPORT double a_shell_versionNumber;
...
extern FILE* a_shell_stdin(void);
extern FILE* a_shell_stdout(void);
extern FILE* a_shell_stderr(void);
extern void* a_shell_context(void);
...
extern int a_shell_setMiniRoot(NSString*);  // restricts operations to a certain hierarchy
...
extern void a_shell_settty(FILE* _tty);
...
extern a_shell_sys_info_t a_shell_getsys_info(void);

// Format system info as string (caller must free)
extern char* a_shell_sysinfo_format(const a_shell_sys_info_t* info);

// Print system info to stdout
extern void a_shell_print_sysinfo(void);

// Get process information for a specific PID
// Returns allocated struct (caller must free with a_shell_freeproc_info), or NULL
extern a_shell_proc_info_t* a_shell_getproc_info(pid_t pid);

// Free process info returned by a_shell_getproc_info
extern void a_shell_freeproc_info(a_shell_proc_info_t* info);

// Get current process info
extern a_shell_proc_info_t* a_shell_getproc_self(void);

// List all running processes
// Returns array of PIDs (caller must free), sets count
extern pid_t* a_shell_list_processes(int* count);

// Format process info as string (caller must free)
extern char* a_shell_procinfo_format(const a_shell_proc_info_t* info);

// Print process info to stdout
extern void a_shell_print_procinfo(pid_t pid);
